...
function syncHeroLine() {
  ...
  const err = state.syncError
    ...
    : "";
  return `<div class="hero-sync"><div>${msg}${err}</div><button class="hero-sync-btn" onclick="retrySync()" aria-label="Sync data">Sync</button></div>`;
}
...
function home() {
  const tx = todayTx(),
    a = todayAtt(),
    c = todayClosing(),
    emptyStockCard = stockEmptyQuickCard();
  if (isDaily()) {
...
function renderLabelEditor() {
  page.innerHTML = `${top("Cetak Label", "KASSEN642BT_904D 40x30")}<iframe src="./cetak_label.html?v=${Date.now()}" style="width:100%;height:calc(100vh - 54px);border:none;background:#f5f5f5;"></iframe>`;
}
